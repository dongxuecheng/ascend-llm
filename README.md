...
两个模型使用相同的 NPU、端口和调度参数，以便进行公平测试。
...
两份 YAML 已把第一张 Atlas 300I Duo 对应的 CPU 固定为 <code>0-31</code>。模型缓存使用独立的 Docker 命名卷，由 Docker 自动创建、持久化并处理 SELinux，无需手工创建缓存目录。
